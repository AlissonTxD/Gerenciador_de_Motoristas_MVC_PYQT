...
def test_register():
    ...
    assert response["driver"] == {"name": "GENIVALDO", "plate": "ABC1D23", "type": "FRETEIRO"}
...
def test_register_name_already_used():
    ...
    assert response["success"] == False
    assert response["error"] == "Erro: Este Nome Já Esta Cadastrado\nNome: CARLOS\nPlaca: ABC1234"

def test_register_plate_already_used():
    response = model.register("genivaldo", "ABC1234", "freteiro", sample_list)
    assert response["success"] == False
    assert response["error"] == "Erro: Esta Placa Já Esta Cadastrada\nNome: CARLOS\nPlaca: ABC1234"